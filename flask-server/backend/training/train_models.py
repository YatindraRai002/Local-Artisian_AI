...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOGS_DIR, "train_models.log"), encoding="utf-8"),
...
class ComprehensiveModelTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        for d in [MODELS_DIR, TRAINING_DATA_DIR, LOGS_DIR, TRAINED_MODELS_DIR]:
    ...
    def generate_training_data(self, num_samples: int = 2000) -> str:
        ...
        # Save training data
        filename = os.path.join(TRAINING_DATA_DIR, f"multilingual_training_data_{self.timestamp}.json")
    ...
    def train_rag_model(self, training_data_path: str) -> str:
        ...
        # Save trained model
        model_path = os.path.join(MODELS_DIR, f"rag_model_{self.timestamp}")
    ...
    def run_comprehensive_training(self, num_samples: int = 2000) -> Dict[str, Any]:
        ...
        try:
            ...
            model_paths = {
                ...
                "classification_models": TRAINED_MODELS_DIR,
            }
    ...
    def save_training_summary(self, results: Dict[str, Any]):
        ...
        summary_path = os.path.join(MODELS_DIR, f"training_summary_{self.timestamp}.json")
        ...
        latest_path = os.path.join(MODELS_DIR, "latest_training_summary.json")
    ...
    def test_trained_models(self):
        ...
        try:
            latest_rag_path = max(
                [d for d in os.listdir(MODELS_DIR) if d.startswith("rag_model_")],
                key=lambda x: x.split("_")[-1],
            )

            rag_model = MultilingualRAGModel()
            rag_model.load_model(os.path.join(MODELS_DIR, latest_rag_path))
            ...
            test_queries = [
                ...
                ("வணக்கம், நெசவு கலைஞர்களைக் காட்டுங்கள்", "tamil"),
            ]
